...
$mpdf->Cell(20,5,'',0,0, '');
	...
	$name = $row['first_name'].' '.$row['others']; $rank = strtoupper($row['staff_rank']);
	...
	<div style="width:100%; height:130px; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19); border-radius: 10px;">
		<img  style="width:100%; height:100%; border-radius: 10px;" src = "staff_imgs/'.$row['photo'].'">
		</div>
	...
	<span style="font-size:11px; font-family: Arial black; color:#003008"><b>'.$name.'</b></span><br>
	<span style="color: white"></span><span style="font-size:15px; color:#3582c4"><b>'.$rank.'</b></span><br>
</div>
	...
	$link_id=str_replace('/','_', $text);
						$qr = $link_id . '-qr.png';
						QRcode::png("www.eforms.ubaznet.com/staff.php?id=$name:- Staff No: ". $rank, $qr, 'H', 10, 10);
	...
	$mpdf->WriteHTML('<div style="background-color: white;text-align:justify; text-justify: inter-word;  width: 100%; height:100%; padding-right: 5%; font-size: 10px;">
		<div style= "padding-top:10px; padding-botton:0px"><h3 style="text-align:center; width:70%; margin-left: auto; margin-right: auto; "> The bearer whose name & photograph appears overleap is a '.$rank.'</h3>
